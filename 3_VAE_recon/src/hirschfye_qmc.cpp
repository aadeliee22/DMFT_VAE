  ...
  spins_(ntau, 1)
{
  ...
  //this->clean_update_(v1, gtau_up_, UP);
  //this->clean_update_(v1, gtau_dw_, DOWN);
}

double HirschFyeQMC::do_montecarlo_step(int * v0, int * v1, const int nperiodSweeps)
{
  for (int i=0; i<N_; i++)
    {
      if (this->single_flip_update_(v0, v1, i))
      {
        nsweeps_ += 1;
...
void HirschFyeQMC::clean_update_(int * v1, std::vector<double> & gtaumat, SPIN_STATE SPIN)
{
  for(int i=0; i<N_; ++i)
    e_ls_[i] = e1lamb_(SPIN*v1[i]);
...
bool HirschFyeQMC::single_flip_update_(int * v0, int * v1, int site)
{
  nchain_ += 1;
  // choose random site
  ...
  bool isSpinFliped = v0[site]*v1[site] < 0;

  // if update successful ...
  if (isSpinFliped)
  {
    // update Green's function
    const double tmp1 = e2lamb_(-v0[site])-1,
      tmp2 = e2lamb_(v0[site])-1,
