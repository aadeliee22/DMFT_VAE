...
public:
  ...
  double do_montecarlo_step(int * v0, int * v1, const int nperiodSweeps);
  void accumulate(double * G_up, double * G_dw, double * G2_up, double * G2_dw);
  void get_auxiliary_fields(int * spins)
  {
    std::memcpy(spins, spins_.data(), sizeof(int)*spins_.size());
    //for (int i=0; i<spins_.size(); i++){ spins[i] = spins_[i];}
  }
...
private:
  ...
  void clean_update_(int * v1, std::vector<double> & gtaumat, SPIN_STATE SPIN);
  bool single_flip_update_(int * v0, int * v1, int i);
