...
class hfqmc:
    ...
    def meas(self, spin, nmeas):
        ...
        spin_prev = np.zeros([self._ntau], dtype = 'int32')
        spin_pres = np.zeros([self._ntau], dtype = 'int32')
        for n in range(nmeas-1):
            spin_prev = spin[n]
            spin_pres = spin[n+1]
            self._impl.do_montecarlo_step(spin_prev, spin_pres, 3)
    ...
    def do_montecarlo_step(self, spin_prev, spin_pres):
        """
          nmcsteps : # of Monte Carlo steps (int)
        """
